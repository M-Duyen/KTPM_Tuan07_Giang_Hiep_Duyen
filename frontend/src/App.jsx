...
import { ShoppingCart, ShoppingBag, Zap, Trash2, Package, Info, X } from 'lucide-react';

const PRODUCT_PU = 'http://localhost:8081/api';
const CART_PU = 'http://localhost:8082/api';
const ORDER_PU = 'http://localhost:8083/api';
const INVENTORY_PU = 'http://localhost:8084/api';
      ...
      <main className="max-w-7xl mx-auto p-6 lg:p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map(product => (
            <div key={product.id} className="group glass rounded-[2rem] overflow-hidden transition-all duration-500 hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-500/10 flex flex-col">
              ...
              <div className="p-6 flex flex-col flex-1">
                <div className="flex justify-between items-start mb-2">
                  <h2 className="text-xl font-bold text-white group-hover:text-blue-400 transition-colors">{product.name}</h2>
                ...
                <div className="flex items-center gap-2 mb-6 mt-auto">
                  ...
                  <span className={`text-xs font-medium uppercase tracking-wider ${stocks[product.id] <= 3 ? 'text-rose-500 animate-pulse' : 'text-slate-500'}`}>
                    {stocks[product.id] > 0 ? `${stocks[product.id]} Units Left` : 'Sold Out'}
                  </span>
                </div>
          ...
          <div className="relative glass max-w-lg w-full rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8">
              <div className="flex justify-between items-end mb-6">
                <div>
                  <span className="flash-badge mb-2 inline-block">Product Detail</span>
                  <h2 className="text-3xl font-bold text-white">{selectedProduct.name}</h2>
                </div>
                <p className="text-3xl font-black text-blue-400">${selectedProduct.price}</p>
              </div>

              <div className="bg-slate-800/50 p-5 rounded-2xl mb-8 border border-white/5">
                <h4 className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mb-2">Description</h4>
              ...
              <div className="flex items-center gap-3 mb-8">
                ...
                <span className={`font-bold ${stocks[selectedProduct.id] <= 3 ? 'text-rose-500' : 'text-emerald-500'}`}>
          ...
          <div className="relative w-full max-w-md bg-slate-900 border-l border-white/5 p-8 flex flex-col shadow-2xl animate-in slide-in-from-right duration-500">
            ...
            <div className="flex-1 overflow-y-auto space-y-6">
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.productId} className="flex justify-between items-center bg-slate-800/40 p-4 rounded-2xl border border-white/5">
                    <div className="text-left">
                      <h4 className="text-white font-bold mb-1">{item.name}</h4>
                      <p className="text-slate-500 text-xs font-medium uppercase tracking-wider">{item.quantity} x ${item.price}</p>
                    </div>
